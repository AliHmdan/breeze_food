...
class DiscountModel {
  ...
  /// ✅ الحقل الجديد للصورة القادمة من API
  ...
  DiscountModel({
    ...
    this.restaurant,
    this.menuItems = const [],
    this.imageUrl, // ✅ تمت إضافته
  });

  factory DiscountModel.fromJson(Map<String, dynamic> json) {
    // ✅ إذا الفورم الأول (coupon_id موجود)
    if (json.containsKey('coupon_id')) {
      return DiscountModel(
        ...
        menuItems: (json['menu_items'] as List<dynamic>? ?? [])
            ...
            .toList(),
        imageUrl: json['image'], // ✅ تمت إضافته هنا أيضاً
      );
    }

    // ✅ إذا فورم home-discount
    return DiscountModel(
      couponId: json['id'] ?? 0,
      code: json['name_en'] ?? json['name_ar'] ?? '',
      discountType: json['discount_type'] ?? '',
      discountValue: (json['discount_value'] is num)
          ? (json['discount_value'] as num).toDouble()
          : double.tryParse(json['discount_value'].toString()) ?? 0.0,
      isAvailableForUser: json['is_favorite'] == true,
      restaurant: json['restaurant_name'] != null
          ? CouponRestaurant(
              id: 0,
